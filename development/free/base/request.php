...
if (
    ...
    and !empty($_POST['email']) and isset($_POST['email']) and !empty($_POST['rules']) and isset($_POST['rules']) and isset($_POST['form_register']) and
    ...
    if ($json->success === $enableRecaptcha) {
        ...
        $login = inputProtec($_POST['login']);
        $email = inputProtec($_POST['email']);
        $password = encryptJ(inputProtec($_POST['password'])); //Crypted password

        if (strlen($login) <= 16 and strlen($email) <= 50 and strlen($_POST['password']) <= 16  and strlen($_POST['password']) >= 4) {

            if (verifyInDb($dbSrv, "accounts", "login", $login) === false and verifyInDb($dbSrv, "accounts", "email", $email) === false) {
...
if (isset($_POST['email']) and !empty($_POST['email']) and isset($_POST['old_password']) and !empty($_POST['old_password']) and isset($_POST['new_password']) and !empty($_POST['new_password']) and isset($_POST['form_change_password']) and isset($_POST['token_user_recaptacha'])) {
    ...
    if ($json->success === $enableRecaptcha) {
        ...
        $result = getEmail($dbSrv, "accounts", "password", "email", $old_pass, $email);

        if (strlen($result) > 0) {

            if (updatePass($dbSrv, "accounts", "email", $result, $new_password) === true) {
